    ...
    void TestApp::UpdateTitle()
    {
        ...
        if (fImageState.GetOpenedImage() != nullptr)
        {
            ...
            case ImageSource::File:
            {
                ...
                {
                    ss << (fCurrentFileIndex == FileIndexStart ? 0 : fCurrentFileIndex + 1) << L"/" << fListFiles.size()
                       << L" | ";
                }
...
#pragma pack(pop)
    ...
    void TestApp::ProcessRemovalOfOpenedFile(const std::wstring &fileName)
    {
        if (fileName == GetOpenedFileName())
        {
            ...
            bool shouldRemoveFile = (internally == true && (fDeletedFileRemovalMode & DeletedFileRemovalMode::DeletedInternally) == DeletedFileRemovalMode::DeletedInternally) || (internally == false && (fDeletedFileRemovalMode & DeletedFileRemovalMode::DeletedExternally) == DeletedFileRemovalMode::DeletedExternally);
