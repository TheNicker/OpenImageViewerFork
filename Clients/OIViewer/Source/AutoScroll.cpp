...
namespace OIV
{
    ...
    void AutoScroll::PerformAutoScroll()
    {
        ...
        const double elapsed = static_cast<double>(
            fAutoScrollStopWatch.GetElapsedTimeReal(StopWatch::TimeUnit::Milliseconds));
        fAutoScrollStopWatch.Start();
        ScrollPointType deltaFromScrollPosition = static_cast<ScrollPointType>(fAutoScrollPosition -
                                                                               GetMousePosition());
        ...
        auto powerX = std::min<double>(
            factorOut, (factorOut - factorIn) / fScrollMetrics.speedFactorRange * deltaAbs.x + factorIn);
        auto powerY = std::min<double>(
            factorOut, (factorOut - factorIn) / fScrollMetrics.speedFactorRange * deltaAbs.y + factorIn);

        auto movementSpeedX = std::min<double>(fScrollMetrics.maxSpeed,
                                               deltaAbs.x == 0 ? 0 : std::pow(deltaAbs.x, powerX));
        auto movementSpeedY = std::min<double>(fScrollMetrics.maxSpeed,
                                               deltaAbs.y == 0 ? 0 : std::pow(deltaAbs.y, powerY));
    ...
    {
        return fCreateParams.window->GetMousePosition();
    }
