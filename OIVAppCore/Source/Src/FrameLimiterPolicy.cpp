...
namespace OIV
{
    ...
    {
        return refreshRateTimes1000 > 0 ? 1'000'000'000 / refreshRateTimes1000 : 0;
    }

    FrameRefreshDecision FrameLimiterPolicy::Decide(bool enabled, bool timerEnabled,
                                                    int64_t microsecondsSinceLastRefresh, int64_t refreshRateTimes1000)
    {
        if (enabled == false)
            return {FrameRefreshAction::RefreshNow};

        const int64_t windowTimeInMicroseconds = FrameWindowMicroseconds(refreshRateTimes1000);
        if (windowTimeInMicroseconds == 0)
            return {FrameRefreshAction::RefreshNow};
